      ...
      for (const log of filteredLogs) {
        const { data: lineItem, error: liError } = await supabase
          ...
          .insert({
            ...
            amount: log.hours * Number(hourlyRate),
      ...
      <form onSubmit={handleSubmit} className="flex flex-col lg:grid lg:grid-cols-[1fr_280px] gap-8">
        ...
        <div className="flex flex-col gap-5">
          ...
          <button
            ...
            className={cx(
              ...
              filteredLogs.length > 0 && !isSubmitting
        ...
        <div className="bg-slate-50 rounded-xl border border-gray-200 p-5 h-fit">
          ...
          <div className="flex justify-between mb-3">
            <span className="text-slate-500 text-sm">Unbilled Logs:</span>
            <span className="font-semibold text-sm">{filteredLogs.length}</span>
          </div>
          <div className="flex justify-between mb-3">
            <span className="text-slate-500 text-sm">Total Hours:</span>
            <span className="font-semibold text-sm">{totalHours}</span>
          </div>
          ...
          {filteredLogs.length > 0 ? (
            ...
            selectedStudentId && selectedTutorId && (
              <p className="text-xs text-red-600 mt-4 leading-relaxed">
